  ...
  try {
    switch (name) {
      case 'web_search': {
        ...
        const data = await res.json();
        return (data.web?.results || []).slice(0, 5).map(r => `â€¢ ${r.title}\n  ${r.description}`).join('\n\n');
      }
      case 'get_weather': {
        ...
        return `${d.location.name}: ${d.current.temp_f}Â°F, ${d.current.condition.text}. 3-day: ${d.forecast.forecastday.map(x => `${x.date}: ${x.day.maxtemp_f}/${x.day.mintemp_f}Â°F`).join(' | ')}`;
      }
      case 'get_news': {
        ...
        const data = await res.json();
        return data.items?.length ? data.items.slice(0, 6).map(i => `â€¢ ${i.title}`).join('\n') : `No news for "${input.topic}".`;
      }
      case 'calculate': {
      ...
      case 'get_canvas_assignments': {
        ...
        if (input.course_id === 'all') {
          ...
          for (const c of courses.slice(0, 8)) {
            ...
            if (Array.isArray(assignments) && assignments.length) {
              all.push(`ðŸ“š ${c.name}:`);
              assignments.slice(0, 6).forEach(a => all.push(`  â€¢ ${a.name} â€” Due: ${a.due_at ? new Date(a.due_at).toLocaleDateString() : 'N/A'} (${a.points_possible} pts)\n    ${(a.description || '').replace(/<[^>]+>/g, '').slice(0, 150)}`));
            }
          }
        ...
        return Array.isArray(assignments) ? assignments.slice(0, 10).map(a => `â€¢ ${a.name} â€” Due: ${a.due_at ? new Date(a.due_at).toLocaleDateString() : 'N/A'}\n  ${(a.description || '').replace(/<[^>]+>/g, '').slice(0, 150)}`).join('\n\n') : `Error: ${JSON.stringify(assignments)}`;
      }
      case 'get_classroom_assignments': {
        ...
        if (input.course_id === 'all') {
          ...
          for (const c of courses.slice(0, 5)) {
            ...
            if (work.length) {
              all.push(`ðŸ“— ${c.name}:`);
              work.slice(0, 5).forEach(w => all.push(`  â€¢ ${w.title} â€” Due: ${w.dueDate ? `${w.dueDate.month}/${w.dueDate.day}/${w.dueDate.year}` : 'N/A'}\n    ${(w.description || '').slice(0, 150)}`));
            }
          }
        ...
        return work.slice(0, 10).map(w => `â€¢ ${w.title} â€” Due: ${w.dueDate ? `${w.dueDate.month}/${w.dueDate.day}` : 'N/A'}\n  ${(w.description || '').slice(0, 150)}`).join('\n\n');
      }
...
wss.on('connection', (ws) => {
  ...
  console.log(`Client connected â€” ${sessionHistory.length} messages, ${Object.keys(persistentMemory).length} memories`);
...
server.listen(PORT, () => {
  console.log(`\nðŸ¤– J.A.R.V.I.S. online at http://localhost:${PORT}`);
  console.log(`ðŸ“¦ ${Object.keys(persistentMemory).length} memories | ðŸ’¬ ${loadHistory().length} history messages`);
  console.log(`ðŸ”‘ Google: ${isGoogleAuthed() ? 'Connected' : 'Not connected â€” visit /auth/google'}`);
  console.log(`ðŸ“š Canvas: ${process.env.CANVAS_API_TOKEN ? 'Configured' : 'Not configured'}\n`);
});
